...
g = nx.readwrite.gml.read_gml("/gpfs_home/spate116/singhlab/GCN_Integration/scripts/BI/path/data/syn_graph_0.gml", label=None)
with open('/gpfs_home/spate116/singhlab/GCN_Integration/scripts/BI/path/data/syn_graph_labels_0.json', 'r') as f:
    labels = json.load(f) 
labels = torch.tensor([labels[str(i)] for i in range(1, len(labels.keys()) + 1)])
x = 0.5 * torch.rand((labels.shape[0], 10), dtype=torch.float32)
x[labels, :] = 1.0
...
if __name__ == '__main__':
    ...
    plt.savefig('/gpfs_home/spate116/singhlab/GCN_Integration/scripts/BI/path/explain/node%d.png' % node_idx, dpi=300)