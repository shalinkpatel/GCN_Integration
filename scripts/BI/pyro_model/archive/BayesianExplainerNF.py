...
class BayesianExplainer():
    ...
    def sample_model(self, X, y):
        if self.sigmoid:
            m = pyro.sample("m", dist.Bernoulli(
                self.flow_dist.rsample(torch.Size([250, ])).sigmoid().clamp(0, 1).mean(dim=0)).to_event(1))
        else:
            m = pyro.sample("m", dist.Bernoulli(
                self.flow_dist.rsample(torch.Size([250, ])).clamp(0, 1).mean(dim=0)).to_event(1))
    ...
    def train(self, epochs: int = 3000, lr: float = 0.005, lambd: float = 0.1, window: int = 1000, p=1.25,
              log: bool = True):
        ...
        loss_fn = lambda model, guide, X, y: pyro.infer.Trace_ELBO().differentiable_loss(model, guide, X,
                                                                                         y) * self.sharp + lambd * self.L(
            self.p)
