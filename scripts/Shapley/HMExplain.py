...
class GCN_Fixed(nn.Module):
    ...
    def forward(self, mask, inputs):
        mask = list(map(lambda x: int(x), mask.cpu().tolist()))
        return self.m(self.G.subgraph(mask), inputs)

# %%
m = GCN_Fixed(model, G)
e = shap.DeepExplainer(m, [torch.tensor(list(range(0, 100))).float(), torch.zeros((100, 1, data.x.shape[1], data.x.shape[2])).float()])

# %%
shap_values = e.shap_values([torch.tensor(list(range(100, 200))).float(), data_embedding[100:200]])


# %%
with open('/gpfs_home/spate116/data/spate116/GCN/%s/res/shapley_0.res' % cl, 'wb') as f:
    pickle.dump({1: shap_values}, f)
